////////////////////////////////////////////////////////////////////
//
// $Id: main.cc 2025/07/19 18:46:55 kanai Exp 
//
...
int main(int argc, char** argv) {
  ...
  // メッシュデータの読み込み
  std::shared_ptr<MeshL> mesh0 = std::make_shared<MeshL>(); // 細分割前のメッシュ
  smflio.setMesh(*mesh0);
  if (smflio.inputFromFile(argv[1]) == false) {
    return EXIT_FAILURE;
  }

  // Loop細分割クラスインスタンスの生成（1回目）
  std::shared_ptr<MeshL> mesh1 = std::make_shared<MeshL>(); // 1回細分割後のメッシュ
  ...
  // 細分割のコードを書いたら，下の行のコメントを外してください．
  //mesh1->calcSmoothVertexNormal();

  // メッシュ表示用 に mesh をセット
  glmeshl.setMesh(mesh0);
  // 細分割のコードを書いたら，下の行のコメントを外し，上の行をコメントしてください．
  // setMesh するのは1つだけにしてください．
